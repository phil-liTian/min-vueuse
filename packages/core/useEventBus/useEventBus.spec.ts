...
import { useCounter } from "@mini-vueuse/shared";
...
describe('useEventBus', () => {
  ...
  it('on event', () => {
    let event = false
    const { on, off, emit, reset } = useEventBus<boolean>('on-event')
    on((_event) => {
      event = _event
    })
  ...
  it('once event', () => {
    ...
    const { on, emit, once } = useEventBus<number>('foo')
    once(inc)
    emit()
    emit()
    emit()
  ...
  it('should work with payload', () => {
    const { emit, on } = useEventBus<'inc' | 'dec', number>('counter')
