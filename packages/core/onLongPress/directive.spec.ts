import { mount, VueWrapper } from '@vue/test-utils'
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { defineComponent } from 'vue'
import { vOnLongPress } from './directive'
import { promiseTimeout } from '@mini-vueuse/shared'
import { onLongPress, OnLongPressOptions } from '.'

const App = defineComponent({
  props: {
    onLongPress: {
      type: Function,
      required: true
    },
    options: {
      type: Object
    }
  },
  template: `<template>
    <div data-test="element" v-on-longpress="onLongPress">Press me</div>
    <div data-test="element-options" v-on-longpress="[onLongPress, options]">Press me</div>
  </template>`
})

describe('vOnLongPress', () => {
  let onLongPress = vi.fn()
  let wrapper: VueWrapper<any>

  describe('given no options', () => {
    beforeEach(() => {
      onLongPress = vi.fn()
      wrapper = mount(App, {
        props: {
          onLongPress
        },
        global: {
          directives: {
            'on-longpress': vOnLongPress
          }
        }
      })
    })

    it('should be defined', () => {
      expect(wrapper).toBeDefined()
    })

    it('should trigger longpress after 500ms', async () => {
      const element = wrapper.get('[data-test=element]')
      await element.trigger('pointerdown')
      await promiseTimeout(500)
      expect(onLongPress).toHaveBeenCalledTimes(1)
    })
  })

  describe('given options', () => {
    beforeEach(() => {
      onLongPress = vi.fn()
      const options: OnLongPressOptions = {
        delay: 1000
      }
      wrapper = mount(App, {
        props: {
          onLongPress,
          options
        },
        global: {
          directives: {
            'on-longpress': vOnLongPress
          }
        }
      })
    })

    it('should be defined', () => {
      expect(wrapper).toBeDefined()
    })

    it('should trigger longpress after 500ms', async () => {
      const element = wrapper.get('[data-test=element-options]')
      await element.trigger('pointerdown')
      await promiseTimeout(500)
      expect(onLongPress).toHaveBeenCalledTimes(0)
      await promiseTimeout(500)
      expect(onLongPress).toHaveBeenCalledTimes(1)
    })
  })
})
